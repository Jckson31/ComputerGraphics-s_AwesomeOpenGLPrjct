...
public:
    // Costruttore 1: Per oggetti semplici senza sedie (es. Il bancone)
    TavernObject(glm::vec2 startPos, glm::vec2 startSize, glm::vec3 startColor);

    // Costruttore 2: Per i tavoli (che hanno le sedie)
    TavernObject(glm::vec2 startPos, glm::vec2 startSize, glm::vec3 startColor, std::vector<glm::vec2> tableSeats);
